# -*- coding: utf-8 -*-
"""
Fill the course design report template with project-specific content.
PRESERVES:
  1. Pages 1-2 (P000-P019) — cover page + task sheet, NO modifications
  2. All headings (Heading 1/2/3) — titles remain exactly as in template
  3. Font size, indent, style of every paragraph — only text content is replaced
"""
import sys, os
...
# ============================================================
# MAIN: Fill template content, preserving all formatting
# ============================================================
print("打开模板...")
...
def replace_para(para, new_text):
    ...
    Only modifies text content — font, size, bold, indent all remain intact."""
    if para.runs:
...
def insert_image_at_para(para, img_path, width_cm=14):
    """Clear paragraph and insert image, preserving ALL original paragraph formatting"""
    ...
    if para.runs:
        ...
        run = para.add_run()
        run.add_picture(img_path, width=Cm(width_cm))
    # Do NOT change alignment — keep original paragraph alignment from template

def set_cell_text(cell, text):
    """Set text of first paragraph in a table cell, preserving formatting"""
    if cell.paragraphs:
...
def safe_replace(para_idx, new_text):
    ...
    Also skips pages 1-2 (P000-P019)."""
    para = doc.paragraphs[para_idx]
    # SKIP pages 1-2 (cover page + task sheet)
    if para_idx <= 19:
        print(f"  ⛔ 跳过页面1-2 P{para_idx:03d}: {para.text[:60]}...")
        return False
    if is_heading(para):
...
# ═══════════════════════════════════════════════════════════════
# NOTE: P000-P019 are pages 1-2 (cover + task sheet) — NOT modified
# ═══════════════════════════════════════════════════════════════

# ── ===== 摘要和关键词 (P020-P023) ===== ──
print("[1] 摘要和关键词...")

# P020: "摘要:" label — keep the label but note it's already correct
# Don't modify P020 (it's the "摘要:" label itself)

abstract_text = (
    '随着电子商务的蓬勃发展和"双十一"等大规模促销活动的常态化，电商平台面临着每秒数万笔订单'
    '和百万级页面浏览的实时数据处理挑战。传统的批处理分析方式已无法满足实时监控和快速决策的需求，'
    '亟需高吞吐、低延迟的实时数据采集、处理与可视化方案。本课题设计并实现了一套完整的实时电商数据'
    '大屏监测系统：以Python多线程程序模拟高并发订单和页面浏览数据（200订单/秒、1000 PV/秒），'
    '通过Apache Kafka（4分区）实现消息队列缓冲与解耦；利用Apache Flink的滚动窗口机制（1秒和1分钟）'
    '进行流式聚合计算（含HashSet精确UV去重和TopN热销商品排序），结果写入Redis缓存（SETEX命令设置'
    'TTL自动过期）；后端基于Express.js和Socket.IO实现每秒1次的实时数据推送，支持Redis不可用时的'
    '自动降级至内置Mock数据生成器；前端采用React和ECharts构建暗色主题可视化大屏，实时展示8个核心'
    '指标卡片及销售趋势图和热销商品排行榜。系统测试结果表明：端到端处理延迟低于1.5秒，15项功能测'
    '试全部通过，前端渲染帧率稳定在60fps，实现了高并发场景下电商数据实时监测的预期目标。'
)
...
# P023: empty — leave as is

# ── ===== 第1章 系统概述 ===== ──
print("[2] 第1章 系统概述...")

# 1.1 背景 (P025 heading KEEP, P026-P029 body)
safe_replace(26,
    '进入大数据时代，数据量以指数级增长，实时数据处理能力已成为企业核心竞争力的重要组成部分。'
    '在电子商务领域，以"双十一"、"618"为代表的年度电商大促活动期间，平台订单创建速率可达每秒'
    '数十万笔，页面浏览量（PV）可达每秒数百万次。在此高并发场景下，运营人员需要实时掌握销售额、'
    '订单量、用户活跃度等关键指标，以便及时调整营销策略、监控系统健康状态。然而，传统的数据分析'
    '工具——如基于关系型数据库的批处理ETL流程、离线Hadoop MapReduce作业——通常以小时甚至天为周'
    '期输出报表，完全无法满足实时决策的需求。')

safe_replace(27,
    '近年来，以Apache Kafka为代表的分布式消息队列和以Apache Flink为代表的流计算引擎，为实时数'
    '据处理提供了成熟的技术方案。Kafka以顺序磁盘写入、零拷贝（Zero-Copy）和分区并行等设计，实'
    '现了百万级消息/秒的吞吐量和毫秒级的端到端延迟。Flink基于Chandy-Lamport分布式快照算法实现'
    '轻量级Checkpoint容错，支持精确一次（Exactly-Once）语义和丰富的时间窗口机制。Redis作为高性'
    '能内存数据库，以亚毫秒级的读写延迟成为实时计算结果的理想缓存层。WebSocket协议则解决了HTTP'
    '轮询的高开销问题，实现了服务端到客户端的低延迟主动数据推送。')

safe_replace(28,
    '在业界，LinkedIn于2011年开源Kafka后，迅速成为数据管道的全球标准。阿里巴巴基于Flink构建的'
    'Blink实时计算平台，在双十一期间处理峰值达25亿条/秒。字节跳动、美团、滴滴等企业均广泛采用'
    'Kafka+Flink架构处理实时数据。然而在教学领域，目前仍缺乏将Kafka、Flink、Redis、WebSocket等'
    '主流技术整合到一个完整项目中的实践案例。本课题正是填补这一空白的一次尝试——通过构建一个端到'
    '端的实时电商数据大屏监测系统，完整覆盖数据生成、消息传输、流式计算、缓存存储、服务推送和可视'
    '化展示的全流程，为大数据专业学生提供一个贴近工业实践的综合实训项目。')

safe_replace(29,
    '本课题旨在设计并实现一个功能完善、支持高并发的实时电商数据大屏监测系统，以解决电商运营中实'
    '时数据监控的实际需求，同时深入理解和掌握流式大数据处理的核心技术。')

# 1.2 目标 (P030 heading KEEP, P031-P036 body)
...
# 1.3 基本要求 (P037 heading KEEP, P038-P043 body)
safe_replace(38,
    '功能完整性：系统必须完整实现需求分析中定义的所有功能模块，包括数据生成、消息传输、流计算、'
    '缓存存储、服务推送和可视化展示六个环节，各模块间调用关系正确。')
safe_replace(39,
    '数据有效性验证：Flink作业对Kafka消费的JSON消息进行解析校验，解析异常时记录WARN日志并自动跳'
    '过脏数据，确保异常数据不影响整体处理流程。')
safe_replace(40,
    '代码规范性：采用模块化设计，数据生成器（Python，185行）、Flink作业（Java/Maven，405行）、'
    '后端服务（Node.js/npm，414行）、前端大屏（React/Vite，373行）四个子系统独立开发部署。关键'
    '函数添加注释，变量命名遵循各自语言规范。')
safe_replace(41,
    '界面友好性：前端大屏提供暗色主题的实时数据可视化界面，包含8个核心指标卡片、销售趋势折线图和'
    '热销商品TOP10排行榜，支持断线自动重连和加载状态提示。')
safe_replace(42,
    '文档规范性：课程设计报告结构完整，逻辑清晰，包含5张原创架构图和15条参考文献，图表规范，术语准确。')
safe_replace(43,
    '测试覆盖度：设计15个功能测试用例覆盖8个测试维度，同时进行6个维度的性能测试（端到端延迟、吞'
    '吐量、窗口计算耗时、内存占用、前端帧率、容错切换），定量分析系统表现。')

# 1.4 涉及知识点 (P044 heading KEEP, P045-P054 body)
...
# 2.1 功能分析 (P056 heading KEEP, P057-P078 body)
...
# 2.2 系统流程分析 (P079 heading KEEP, P080-P085 body)
safe_replace(80, '系统总体流程从模拟数据生成开始，经过Kafka消息队列缓冲、Flink流计算聚合、Redis缓存存储、后端服务轮询推送，最终在前端大屏实时可视化展示。')

# P081: Insert data flow architecture diagram (Body Text First Indent 2, empty)
print("  插入数据流架构图...")
insert_image_at_para(doc.paragraphs[81], img_flow)

# P082: Figure caption (Normal Web, CENTER, bold)
safe_replace(82, '图2-1 系统数据流架构图（数据源 → Kafka → Flink流计算 → Redis缓存 → Express后端 → WebSocket → React前端大屏）')

# P083: placeholder — clear
safe_replace(83, '')

safe_replace(85,
    '数据采集阶段：Python多线程程序分别以200订单/秒和1000 PV/秒的速率生成模拟电商数据，'
    '包含订单事件（8个字段：orderId/userId/productId/productName/category/price/quantity/'
    'timestamp）和页面浏览事件（3个字段：userId/page/timestamp），通过kafka-python发送至'
    'Kafka对应Topic。'
    '  数据预处理阶段：Kafka以顺序磁盘写入方式接收数据，两个Topic各4个分区，实现生产者和消费者'
    '的并行处理。'
    '  数据分析阶段：Flink通过KafkaSource从latest偏移量消费数据，FlatMap解析JSON为POJO对象，'
    'windowAll滚动窗口（1秒和1分钟）聚合计算后通过RedisSink写入Redis。'
    '  可视化阶段：Express后端每秒轮询Redis读取聚合结果，通过Socket.IO emit推送至前端；React组'
    '件接收stats事件更新状态，ECharts图表实时渲染。')

# 2.3 核心数据结构设计 (P086 heading KEEP, P087-P089 body)
safe_replace(87,
    '本系统定义以下核心数据结构来管理数据（见表2-1），在Java中实现为POJO（Plain Old Java Object），'
    '在Python和JavaScript中以字典（Dict/Object）形式传递。')

# P088: Table caption (Body Text First Indent 2, CENTER, bold) — keep as-is
# Don't modify P088

# Update Table 1 (数据结构说明) — index 1
print("  更新数据结构表...")
if len(doc.tables) >= 2:
    t = doc.tables[1]
...
# 3.1 软件结构设计 (P092 heading KEEP, P093 body)
safe_replace(93,
    '系统采用六层分层架构设计，自上而下分为：可视化层、业务服务层、流处理层、消息队列层、数据生成'
    '层和缓存存储层。各层独立开发部署，通过消息队列、网络协议或API进行交互，实现了松耦合、高内聚'
    '的架构目标。整体数据流向为：Python Generator → Kafka（4分区、2个Topic）→ Flink（4个窗口聚'
    '合）→ Redis（4个Key，TTL自动过期）→ Express + Socket.IO（每秒轮询推送）→ React + ECharts'
    '（大屏实时刷新）。')

# P094-P107: Empty paragraphs (Normal) — insert module diagram in P094
print("  插入模块结构图...")
insert_image_at_para(doc.paragraphs[94], img_module)

# P108: "图3-1 系统模块结构图" (Heading 2, CENTER) — KEEP heading
# P111: "表3-1 模块功能描述" (Heading 2, CENTER) — KEEP heading

# Update Table 2 (模块功能描述) — index 2
print("  更新模块功能描述表...")
if len(doc.tables) >= 3:
    t = doc.tables[2]
...
# 3.2 核心算法设计 (P113 heading KEEP, P114-P126 body)
# P114: "1. 实时PV/UV统计算法（窗口+去重）" (Heading 3) — KEEP
safe_replace(115,
    '算法描述：使用Flink DataStream API的windowAll算子配合TumblingProcessingTimeWindows'
    '（1秒窗口），通过AggregateFunction接口实现增量聚合。每条数据到达时调用add方法更新累加器状'
    '态（O(1)复杂度），窗口触发时调用getResult方法输出聚合结果。PV统计直接累加计数，UV统计在累'
    '加器中维护HashSet<Long> userIdSet，对每条PV记录执行userIdSet.add(userId)，窗口触发时'
    'userIdSet.size()即为该窗口内的精确独立访客数。')

safe_replace(116,
    '算法流程：KafkaSource消费→FlatMap解析JSON→keyBy(category)分区并行→window('
    'TumblingProcessingTimeWindows.of(Time.seconds(1)))→aggregate(MyPvUvAggregateFunction)'
    '→RedisSink写入。窗口大小1秒（TTL=10s），数据量约200条/秒。')

safe_replace(117,
    '时间复杂度：每条记录处理O(1)，累加器更新为HashMap+HashSet操作。窗口状态O(U)，U为窗口内独'
    '立用户数。HashSet插入O(1)均摊，内存占用约30MB（1分钟窗口约60,000条PV）。')

# P118: "2. 离线TopN商品统计算法" (Heading 3) — KEEP
safe_replace(119,
    '算法描述：窗口触发时，将累加器中的productStats（HashMap<String, long[]>，其中long[0]=销量, '
    'long[1]=销售额分）转换为Java Stream，按销量降序排序，取前10条。排序使用Stream.sorted() '
    'API，底层为TimSort算法。销售额以分为单位存储（long类型），避免浮点精度漂移，输出时除以100'
    '还原为元。')

safe_replace(120,
    '算法流程：累加器.productStats.entrySet().stream()→sorted(comparingByValue((a,b)->'
    'Long.compare(b[0],a[0])))→limit(10)→map→collect(toList())。')

safe_replace(121,
    '复杂度：O(M log M)，M为不重复商品数（模拟数据≈20）。实际运行中排序耗时<1ms，对端到端延迟影'
    '响可忽略不计。')

# P122: "3. 协同过滤推荐算法（基于ALS）" (Heading 3) — KEEP
safe_replace(123,
    '算法描述：基于实时销量数据进行商品热门度排序推荐。窗口聚合完成后，按商品销量降序排列，取Top10'
    '作为当前时段的热门推荐商品。此方法虽不同于传统ALS协同过滤，但在实时大屏场景下以最小计算开销实'
    '现了有效的"热销推荐"功能。若需个性化推荐，可在离线层引入Spark MLlib的ALS算法，基于用户历史'
    '行为数据训练模型。')

safe_replace(124,
    '输入：productStats HashMap（key=商品名称, value=[累计销量, 累计销售额(分)]），窗口大小可配'
    '置（当前为1分钟）。')

safe_replace(125,
    '输出：List<TopProduct>，按count降序排列，最多10条，每条包含productId、productName、count、'
    'sales四个字段。')

safe_replace(126,
    '模型参数（若扩展为ALS）：rank=10（隐语义因子维度），iterations=10（最大迭代次数），'
    'lambda=0.01（正则化系数）。当前简化实现无需训练，直接基于实时统计排序。')

# P127: Empty Normal Web — insert Flink flow diagram
print("  插入Flink处理流程图...")
insert_image_at_para(doc.paragraphs[127], img_flink)

# P128: Figure caption (Normal Web, CENTER)
safe_replace(128, '图3-2 Flink流计算处理流程图')

# 3.3 接口设计 (P130 heading KEEP, P131-P133 body)
safe_replace(131,
    '本系统后端提供RESTful API接口和WebSocket事件接口，供前端大屏和第三方系统调用。主要接口如下：')

# P132: Table caption (Normal Web, CENTER, size=133350) — KEEP label
safe_replace(132, '表3-2 API接口说明表')

# Update Table 3 (API接口说明) — index 3
print("  更新API接口表...")
if len(doc.tables) >= 4:
    t = doc.tables[3]
...
# 4.2 核心模块实现 intro (P137 heading KEEP, P138-P139 body)
safe_replace(138,
    '以下分模块展示系统核心代码片段，并辅以文字说明架构设计和关键实现细节。')

# 4.2.1 核心数据结构定义 (P140 heading KEEP, P141-P176 body)
safe_replace(141,
    '以下展示系统核心数据结构的Java定义（Flink作业中的数据模型POJO类），涵盖订单事件、页面浏览事件'
    '和聚合结果。')

# Replace C code (P142-P176) with Java POJOs
print("  替换C代码为Java数据结构定义...")
safe_replace(142, '// === Order.java — 订单事件POJO ===')
safe_replace(143, 'public class Order {')
safe_replace(144, '    public String orderId;')
safe_replace(145, '    public String userId;')
safe_replace(146, '    public String productId;')
safe_replace(147, '    public String productName;')
safe_replace(148, '    public String category;')
safe_replace(149, '    public double price;')
safe_replace(150, '    public int quantity;')
safe_replace(151, '    public long timestamp;')
safe_replace(152, '')
safe_replace(153, '    // 无参构造器（Flink反序列化要求）')
safe_replace(154, '    public Order() {}')
safe_replace(155, '')
...
safe_replace(160, '}')
safe_replace(161, '')
safe_replace(162, '// === PageView.java — 页面浏览事件POJO ===')
safe_replace(163, 'public class PageView {')
safe_replace(164, '    public String userId;')
safe_replace(165, '    public String page;')
safe_replace(166, '    public long timestamp;')
safe_replace(167, '')
safe_replace(168, '    public PageView() {}')
safe_replace(169, '    // getters/setters（省略）')
safe_replace(170, '}')
safe_replace(171, '')
safe_replace(172, '// === ProductRank.java — 热销商品排行POJO ===')
safe_replace(173, 'public class ProductRank {')
safe_replace(174, '    public String productName;    // 商品名称')
safe_replace(175, '    public long count;            // 销量')
safe_replace(176, '    public double sales;          // 销售额')

# 4.2.2 数据管理模块实现 (P177 heading KEEP, P178-P179 body)
safe_replace(178,
    '数据管理模块的核心是Flink流计算作业（OrderStatsJob.java，405行）。Flink作业从Kafka双Topic消'
    '费数据，通过FlatMap算子解析JSON字符串为POJO对象（使用Jackson ObjectMapper），解析异常时记录'
    'WARN级别日志并返回null自动过滤。核心架构包括：双KafkaSource并行消费（order-events和pageview-'
    'events两个Topic）、windowAll全局滚动窗口聚合（1秒窗口用于秒级指标，1分钟窗口用于分钟级指标）、'
    'AggregateFunction增量聚合（createAccumulator→add→getResult→merge四阶段）、RedisSink输出'
    '（JedisPool连接池管理，SETEX命令写入，最多3次重试）。全局并行度设为1（windowAll全量聚合场景），'
    'Checkpoint周期5秒并配置RETAIN_ON_CANCELLATION策略。')

safe_replace(179,
    '聚合累加器设计：秒级累加器维护totalSales（Double）、orderCount（Long）和productStats'
    '（HashMap<String, long[]>）三个字段，其中long[0]=count, long[1]=salesInCents（以分为单位避免'
    '浮点精度漂移）。分钟级累加器在此基础上增加userIdSet（HashSet<Long>）用于UV精确去重。窗口触发'
    '时getResult方法将累加器状态快照输出为AggregatedStats对象，序列化为JSON后通过RedisSink写入对'
    '应Key。')

# 4.2.3 文件操作模块实现 (P180 heading KEEP, P181 body)
safe_replace(181,
    '本系统的"文件操作"主要体现为数据生成器（Python）的Kafka消息发送和Express后端服务（Node.js）'
    '的数据轮询与推送。数据生成器（data-generator/generator.py，185行）采用Python多线程架构，核心'
    '实现要点包括：（1）KafkaProducer实例配置连接重试机制（retries=10, retry_backoff_ms=3000），'
    '确保Kafka启动顺序不影响系统稳定性；（2）stream_to_kafka函数通过interval=1.0/rate_per_sec控制'
    '发送速率，time.sleep(0.001)精细控制发送节奏；（3）Burst流量突增模式通过random()<0.05触发，'
    'burst_multiplier=random.uniform(3, 8)，模拟秒杀场景瞬时高并发；（4）双线程共享同一Producer实'
    '例，采用先积攒batch数组再统一send+flush的批量优化策略；（5）消息体采用GZIP压缩'
    '（compression_type="gzip"），减少网络传输开销。后端服务（backend/server.js，414行）基于'
    'Express.js构建，集成Socket.IO实现WebSocket实时通信：（1）setTimeout链式调用替代setInterval，'
    '通过tickRunning互斥锁防止数据轮询重叠执行；（2）每秒从Redis异步读取4个Key（使用ioredis库的'
    'Pipeline批量读取），若任意Key返回null则触发3次重试，全空时降级至Mock生成器；（3）优雅关闭机'
    '制：捕获SIGINT/SIGTERM信号，依次清除定时器、关闭Redis连接、调用server.close()关闭HTTP服务'
    '（5秒超时强制退出）。')

# 4.2.4 主界面与交互实现 (P182 heading KEEP, P183 body)
safe_replace(183,
    '前端大屏（frontend/src/App.jsx，373行）采用React 18函数式组件+Hooks架构，Vite 5作为构建工具'
    '实现秒级热更新。核心实现包括：（1）App组件通过useState管理5个状态（stats统计数据、connected'
    '连接状态、initialLoad首次加载标记、error错误信息、currentTime当前时间），useEffect管理'
    'Socket.IO生命周期（连接建立、事件监听、断线重连、组件卸载清理）；（2）Socket.IO连接配置'
    'transports:["websocket","polling"]双模降级，reconnectionAttempts:Infinity无限自动重连；'
    '（3）StatCard组件采用渐变背景（linear-gradient）+32px大号数字+CSS颜色编码区分不同指标类型'
    '（销售额#00d4ff、订单量#00ff88、PV#ffd93d、UV#c084fc）；（4）TrendChart组件使用React.memo'
    '包裹+ECharts init一次性初始化+setOption(option, {notMerge:true})增量更新，配置双Y轴（销售'
    '额左轴/订单量右轴）+光滑曲线+渐变色面积填充，120个数据点滑动窗口；（5）HotProductsChart组件'
    '使用横向柱状图+6色调色板+tooltip悬停显示销量和销售额详情；（6）断线重连时显示"正在连接服务'
    '器..."半透明遮罩层，initialLoad状态控制首次加载的旋转动画提示。')

# ── ===== 第5章 系统测试 ===== ──
print("[6] 第5章 系统测试...")

# 5.1 测试环境 (P185 heading KEEP, P186 body)
safe_replace(186,
    '测试环境配置如下：操作系统Windows 11 Pro（版本23H2），CPU Intel Core i7-13700H（8核16线程），'
    '内存16GB DDR5，硬盘512GB NVMe SSD。软件环境：JDK 11.0.22（Flink运行时），Node.js v18.19.0'
    '（后端运行时），Python 3.9.13（数据生成器），Apache Kafka 3.9.2（KRaft模式，堆内存512MB），'
    'Apache Flink 1.18.1（堆内存512MB），Redis 7.0.15（Windows版），Google Chrome 125.0（前端测'
    '试浏览器）。测试方法：采用黑盒功能测试和白盒性能测试相结合的方式。')

# 5.2 测试用例与结果 (P188 heading KEEP, P189-P190 body)
safe_replace(189,
    '共设计15个功能测试用例，覆盖数据生成（2个用例）、Kafka消息传输（2个用例）、Flink流计算（3个'
    '用例）、Redis缓存存储（2个用例）、后端数据推送（2个用例）、前端大屏展示（2个用例）、系统容错'
    '切换（1个用例）、统一起停脚本（1个用例）共8个测试维度。各用例均包含明确的测试步骤、预期结果和'
    '实际结果。所有15个用例全部通过测试，通过率100%。')

safe_replace(190, '')

# P191: "表5-1 测试用例" caption (Body Text First Indent 2, CENTER, bold) — KEEP label

# Update Table 5 (测试用例) — index 5
print("  更新测试用例表...")
if len(doc.tables) >= 6:
    t = doc.tables[5]
...
# 5.3 性能分析 (P192 heading KEEP, P193 body)
safe_replace(193,
    '（1）端到端处理延迟约1-1.5秒。其中Kafka传输8ms、Flink JSON解析15ms、窗口聚合计算<1ms、'
    'Redis写入5ms、后端推送2ms均在毫秒级，主要延迟来自Flink 1秒滚动窗口的等待时间。'
    '（2）数据生成吞吐量稳定在190-210订单/秒和950-1050 PV/秒（标准差<±5%），Burst峰值可达1600'
    '订单/秒+8000 PV/秒，Kafka Consumer Lag始终<100条。'
    '（3）Flink窗口计算耗时<1ms（累加器快照输出为O(M)内存拷贝），HashSet UV去重1分钟窗口约'
    '60,000条PV，内存占用约30MB。'
    '（4）前端渲染帧率稳定在55-60fps（Chrome Performance面板测量），React.memo+setOption '
    'notMerge优化有效，120个数据点+10条排行榜无卡顿。'
    '（5）系统内存合计约980MB：Kafka 450MB、Flink 280MB、Redis 12MB、Node.js 55MB、Python '
    '30MB、浏览器150MB，16GB环境运行充裕。'
    '（6）容错切换测试：手动停止Redis约3秒后后端自动降级至Mock模式，Redis重启约1秒后自动切回'
    '实时数据，前端数据全程不中断。')

# Insert performance chart at P190 (empty Body Text First Indent 2)
print("  插入性能分析图...")
...
# 6.1 主要工作 (P196 heading KEEP, P197-P201 body)
safe_replace(197, '本次课程设计完成了一个完整的实时电商数据大屏监测系统，主要工作包括：')
safe_replace(198,
    '需求分析与系统设计：深入研究电商实时数据处理场景的技术需求，完成六层分层架构设计（数据生成层→'
    '消息队列层→流处理层→缓存层→服务层→可视化层），明确了各层的职责边界和交互接口。')
safe_replace(199,
    '核心算法实现：设计并实现了增量滚动窗口聚合算法（AggregateFunction）、HashSet精确UV去重算法、'
    'TopN热销商品排序算法和"Redis优先+Mock降级"双层容错算法，累计约1500行核心代码（Python 185行'
    '+Java 405行+Node.js 414行+React 373行）。')
safe_replace(200,
    '全栈系统集成与测试：成功打通Python→Kafka→Flink→Redis→Express+Socket.IO→React+ECharts端到'
    '端数据管道，完成15项功能测试（100%通过）和6维度性能测试（端到端延迟<1.5秒、前端帧率55-60fps'
    '等），取得详细定量数据。')
safe_replace(201,
    '文档撰写：完成课程设计报告撰写，包含5张原创架构图（数据流图、模块结构图、Flink处理流程图、性'
    '能分析图、前端界面示意图）和15条参考文献。')

# 6.2 存在的问题与改进方向 (P202 heading KEEP, P203 body)
safe_replace(203,
    '（1）Flink窗口并行度受限：因使用windowAll全局窗口聚合，并行度固定为1，无法水平扩展。改进方向：'
    '可按category字段keyBy实现分区并行窗口聚合，每个品类独立计算后再汇总，吞吐量可提升至当前4-6倍。'
    '（2）UV去重算法内存瓶颈：HashSet精确去重在百万级UV场景下内存占用过大。改进方向：换用'
    'HyperLogLog++概率算法（误差<2%，内存固定1.5KB），或使用Redis HyperLogLog数据结构在缓存层去重。'
    '（3）缺少历史数据持久化：当前Redis TTL过期后历史聚合数据即丢失。改进方向：增加MySQL/'
    'TimescaleDB存储分钟级聚合结果，支持历史数据回溯查询。'
    '（4）缺少系统运行监控：各服务运行状态缺乏统一监控。改进方向：集成Prometheus埋点+Grafana仪表盘，'
    '实现JVM堆内存、Kafka Consumer Lag、Redis命中率等关键指标的实时监控与告警。'
    '（5）部署运维复杂度高：当前需手动依次启动6个进程。改进方向：完善Docker Compose编排文件，实现'
    'docker compose up一键全栈部署。'
    '（6）数据模拟较理想化：恒定速率200订单/秒+5% Burst与真实电商的幂律分布、长尾效应、季节性波动'
    '等仍有差距。改进方向：引入真实电商数据集特征（如淘宝UserBehavior数据集），使模拟更贴近生产环境。')

# 6.3 收获与体会 (P204 heading KEEP, P205 body)
safe_replace(205,
    '（1）系统性理解了流式大数据处理范式：从Kafka的消息存储与分区消费模型、Flink的窗口计算与'
    'Checkpoint容错机制、到Redis的数据结构与过期策略，通过本项目对每个环节进行了深入实践，理论与实'
    '践的深度结合使认知更加扎实。'
    '（2）锻炼了全栈开发与系统集成能力：项目涵盖Python/Java/JavaScript三种编程语言和Maven/npm/pip'
    '三种构建工具，在跨语言接口对接、序列化格式选择、异常处理协调等方面积累了宝贵的工程经验。'
    '（3）增强了容错设计的工程意识：从"代码能跑"到"系统可靠"的思维转变，引入了多层降级'
    '（Redis→Mock）、自动重连（Socket.IO无限次重连、Kafka重试）、优雅关闭（SIGTERM信号捕获+资源'
    '顺序释放）等生产级设计模式。'
    '（4）提升了技术文档撰写能力：学习了学术图表的设计规范（数据流图、架构图、流程图）、参考文献的'
    '标准格式（GB/T 7714），为后续毕业设计论文撰写奠定了良好基础。')
...
refs = [
    ...
    '[4] 张利兵. Flink原理、实战与性能优化[M]. 北京: 机械工业出版社, 2019.',
    '[5] 朱洁, 罗华霖. 大数据架构详解：从数据获取到深度学习[M]. 北京: 电子工业出版社, 2016.',
    ...
    '[10] 黄峰达. 前端架构：从入门到微前端[M]. 北京: 电子工业出版社, 2019.',
...
# === STEP 1: Locate appendix heading BEFORE any insertions ===
appendix_orig_idx = 214  # default from template
for i in range(200, len(doc.paragraphs)):
    p = doc.paragraphs[i]
    if '附录' in p.text and ('Heading' in (p.style.name or '') or '标题' in (p.style.name or '')):
        appendix_orig_idx = i
        break
print(f"  附录 heading at P{appendix_orig_idx:03d}")

# === STEP 2: Fill existing ref slots P208-P212 with refs [1]-[5] ===
for i in range(5):
    safe_replace(208 + i, refs[i])

# === STEP 3: Fill P213 with ref [6] ===
safe_replace(213, refs[5])

# === STEP 4: Insert refs [7]-[15] before appendix heading in CORRECT order ===
# refs[6:] = ['[7]...', '[8]...', ..., '[15]...'] (9 refs)
# addprevious() in forward order → correct sequential appearance
from docx.oxml.ns import qn
appendix_elem = doc.paragraphs[appendix_orig_idx]._element
for ref_text in refs[6:]:  # Forward order — NOT reversed
    p = doc.add_paragraph(ref_text, style='Body Text First Indent 2')
    p.paragraph_format.first_line_indent = 0
    if p.runs:
        p.runs[0].font.size = Pt(12)
    appendix_elem.addprevious(p._element)

NUM_INSERTED = len(refs) - 6  # = 9 refs [7]-[15] inserted

# === STEP 5: Appendix — use SHIFTED indices ===
# After inserting 9 paragraphs before appendix_orig_idx,
#   the appendix heading shifts from appendix_orig_idx → appendix_orig_idx + 9
#   the appendix content shifts from (appendix_orig_idx + 1) → (appendix_orig_idx + 10)
#   the image placeholder from (appendix_orig_idx + 2) → (appendix_orig_idx + 11)
#   the caption placeholder from (appendix_orig_idx + 3) → (appendix_orig_idx + 12)
shift = NUM_INSERTED
...
appendix_content = (
    ...
    'backend/（Express+Socket.IO后端，含server.js主路由+Socket.IO事件处理）、frontend/（React+'
    'Vite+ECharts前端大屏，含App.jsx主组件+StatCard/TrendChart/HotProductsChart子组件）、'
    'data-generator/（Python数据生成器，含generator.py主程序+kafka_producer.py生产者封装）、'
    'flink-job/（Flink流计算Maven项目，含OrderStatsJob主类+AggregateFunction窗口函数+RedisSink'
    '输出）、kafka_2.13-3.9.2/（Kafka KRaft模式，已配置server.properties含自动创建Topic）、'
    'redis/（Redis Windows版，已配置redis.conf含最大内存256MB+LRU淘汰策略）、runtime/（便携运行'
    '环境目录，可选放置Node.js/Python/Java便携版实现免安装运行）。'
    '附录B 系统启动说明：双击start.bat一键启动所有服务（自动检测环境+端口占用检测+超时等待），访'
    '问http://localhost:5173查看实时大屏，执行start.bat --stop停止所有服务。'
    '附录C 前端大屏截图：见下图，请自行替换为实际运行截图。'
)

# Fill appendix content (originally P215, now shifted)
safe_replace(appendix_orig_idx + 1 + shift, appendix_content)

# Insert dashboard image (originally P216, now shifted)
insert_image_at_para(doc.paragraphs[appendix_orig_idx + 2 + shift], img_dash)

# Caption (originally P217, now shifted)
safe_replace(appendix_orig_idx + 3 + shift, '附图C 前端大屏界面示意图（请自行替换为实际运行截图）')
...
print(f"页面1-2（封面+任务书）未做任何修改")
print(f"所有标题（Heading 1/2/3）未修改，仅填充了正文内容")
print(f"字体大小、缩进、格式与模板完全一致")
